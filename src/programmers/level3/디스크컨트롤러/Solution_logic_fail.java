...
public class Solution_logic_fail {
    ...
    public int solution(int[][] jobs) {
        int answer = 0;
        PriorityQueue<Process> waitingJobs = new PriorityQueue<>();
        for (int i = 0; i < jobs.length; i++) {
            waitingJobs.add(new Process(jobs[i][0], jobs[i][1]));
        }

        PriorityQueue<Process> pQueue = new PriorityQueue<>();
        pQueue.add(waitingJobs.poll());
        
        int seconds = 0;
        while (!pQueue.isEmpty()) {
            Process process = pQueue.poll();
            seconds += process.getDuration();
            
            while (!waitingJobs.isEmpty()) {

                Process next = waitingJobs.peek();
                if (next.getRequested() < seconds) {
                    waitingJobs.poll();

                    int end = Math.abs(next.getDuration() - next.getRequested());
                    next.setEnd(end);
            ...
            int time = seconds - process.getRequested();
            answer += time;
            
            if (pQueue.isEmpty() && !waitingJobs.isEmpty()) {
                Process next = waitingJobs.poll();
